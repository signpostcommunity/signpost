  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', minHeight: 'calc(100vh - 73px)', padding: '40px 24px' }}>
      <div style={{ maxWidth: 420, width: '100%' }}>
        <h1 style={{ fontFamily: "'DM Sans', sans-serif", fontSize: '1.8rem', fontWeight: 700, letterSpacing: '-0.03em', marginBottom: '8px' }}>Sign In</h1>
        <p style={{ color: 'var(--muted)', marginBottom: '32px', fontSize: '0.9rem' }}>
          Welcome back.{' '}
          <Link href="/request/signup" style={{ color: 'var(--accent)', textDecoration: 'none' }}>Create an account</Link>
        </p>
        ...
        <form onSubmit={handleLogin} style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
          ...
          <AuthInput label="Password" type="password" value={password} onChange={setPassword} placeholder="Your password" />
          <button type="submit" disabled={loading} className="btn-primary btn-large" style={{ marginTop: '8px', opacity: loading ? 0.7 : 1 }}>
            {loading ? 'Signing in…' : 'Sign In →'}
          </button>
        </form>
        <p style={{ marginTop: '20px', textAlign: 'center', fontSize: '0.82rem' }}>
          <Link href="/request" style={{ color: 'var(--muted)', textDecoration: 'none' }}>← Back to request portal</Link>
        </p>
      </div>
    </div>
  );
